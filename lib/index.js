 ...
 * prompt 非空、autoSend 布尔归一（缺省 false，非布尔抛错）。
...
export function normalizeButtons(raw) {
  ...
  for (const item of list) {
    ...
    if (seen.has(name)) continue;
    seen.add(name);
    out.push({ name, prompt, autoSend: item.autoSend === true });
 ...
 * 类型不对直接抛错（注册与写入校验都走这条路径）。
 * @returns {(value: unknown) => { directories: string[], commands: Array<{name,prompt}>, buttons: Array<{name,prompt,autoSend}> }} schema 函数。
 */
